...
from examples.depth_wan.transformer_depth import (
    DepthAIDNs,
    DepthWanModel,
    TrajectoryFPNHead,
)
from examples.depth_wan.robodataset import MiniFlowRoboDepth
...
def main():
    ...
    rgb_transformer.requires_grad_(False)
    ## depth dit
    depth_transformer = copy.deepcopy(rgb_transformer)

    # modify input channels
    # old_patch = rgb_transformer.patch_embedding
    # rgb_transformer.patch_embedding = torch.nn.Conv3d(
    #     old_patch.in_channels + 16,
    #     old_patch.out_channels,
    #     old_patch.kernel_size,
    #     old_patch.stride,
    #     old_patch.padding,
    # )
    ...
    trajectory_head = TrajectoryFPNHead(
        rgb_transformer.dim, 16, num_tensors=30, patch_size=2
    )
    trajectory_head.to(accelerator.device)
    ...
    transformer_rgb = get_peft_model(rgb_transformer, rgb_lora_config)
    transformer_depth = get_peft_model(depth_transformer, depth_lora_config)

    transformer_rgb.print_trainable_parameters()
    transformer_depth.print_trainable_parameters()
    transformer_rgb.train()
    transformer_depth.train()

    model_transformer = DepthWanModel(
        transformer_rgb, transformer_depth, aidns, trajectory_head
    )
    params_optim = filter(lambda p: p.requires_grad, model_transformer.parameters())

    ## prepare dataset
    dataset = MiniFlowRoboDepth(data_root=args.data_path, is_train=True, max_num_frames=49)
    ...
    while True:
        ...
        for step, batch in enumerate(dataloader):
            ...
            flow = batch["flow"].to(dtype=weight_type)
            ...
            # traj_hidden_states is for traj_loss
            flow_target = pipe.vae.encode(flow, device=accelerator.device).to(
                dtype=weight_type, device=accelerator.device
            )
            ...
            rgb_noise = torch.randn_like(rgb_latent)
            depth_noise = torch.randn_like(depth_latent)
            ...
            rgb_out, depth_out, flow_out = model_transformer(
                noisy_rgb_latents,
                noisy_depth_latents,
                timestep,
                prompt_embed,
            ...
            assert flow_target.shape == flow_out.shape, "flow shape not match"
            ...
            flow_loss = F.mse_loss(
                flow_out.float(), flow_target.float(), reduction="mean"
            )
            tot_loss = rgb_loss + depth_loss + 0.5 * flow_loss
            ...
            avg_flow_loss = accelerator.gather(flow_loss).mean()
            ...
            if accelerator.is_local_main_process:
                if global_step % args.save_steps == 0:
                    ...
                    model = accelerator.unwrap_model(model_transformer)
                    rgb_transformer = model.rgb_transformer
                    depth_transformer = model.depth_transformer
                    aidns_modules = model.aidns
                    traj_modules = model.trajectory_head

                    rgb_transformer.save_pretrained(
                        os.path.join(out_dir, "rgb_transformer_lora")
                    )
                    depth_transformer.save_pretrained(
                        os.path.join(out_dir, "depth_transformer_lora")
                    )
                    aidns_modules.save_pretrained(
                        os.path.join(out_dir, "aidns_modules")
                    )
                    traj_modules.save_pretrained(
                        os.path.join(out_dir, "traj_modules")
                    )
