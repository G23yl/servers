...
from examples.kairos_based.modules.utils import FLAGS_KAIROS_CUDA_SM
...
from examples.kairos_based.third_party.fla.layers.gated_deltanet_with_tp import (
    GatedDeltaNet as GatedDeltaNetWithTP,
)
from fla.models.utils import Cache as fla_Cahce
import torch.distributed as dist
from examples.kairos_based.modules.utils import parallel_state, FLAGS_KAIROS_IS_METAX
from examples.kairos_based.modules.utils.linear import (
...
def rope_apply_for3d_triton(x, num_frames, freqs, num_heads):
    ...
    H = num_heads
    x = rearrange(
        x, "b s (n d) -> b s n d", n=num_heads
    )  # [B,T,H,HD] ([1,720000,20,128]) bf16
    HD = x.shape[-1]
    C = HD // 2  # 64
...
def flash_attention(
    ...
    if compatibility_mode or attn_mask is not None:
        ...
        x = flash_attn_interface.flash_attn_func(
            q, k, v, window_size=window_size, return_attn_probs=return_attn_probs
        )
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        modal_q, modal_k, modal_v = (
            modal_dit_layer.self_attn.norm_q(modal_dit_layer.self_attn.q(modal_x)),
            modal_dit_layer.self_attn.norm_k(modal_dit_layer.self_attn.k(modal_x)),
            modal_dit_layer.self_attn.v(modal_x),
        )
        modal_q = rope_apply(modal_q, modal_freqs, modal_dit_layer.num_heads)
        modal_k = rope_apply(modal_k, modal_freqs, modal_dit_layer.num_heads)

        q = self.norm_q(self.q(x))
        k = self.norm_k(self.k(x))
        v = self.v(x)
        q = rope_apply(q, video_freqs, self.num_heads)
        k = rope_apply(k, video_freqs, self.num_heads)
        use_dilated = dilated_length > 1 and x.shape[1] // (dilated_length * L) > 1
        ...
        if use_dilated:
            ...
            if pad_len != 0:
                ...
                k = F.pad(k, (0, 0, 0, pad_len))
                v = F.pad(v, (0, 0, 0, pad_len))
                modal_q = F.pad(modal_q, (0, 0, 0, pad_len))
                modal_k = F.pad(modal_k, (0, 0, 0, pad_len))
                modal_v = F.pad(modal_v, (0, 0, 0, pad_len))
            q = rearrange(q, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length)
            k = rearrange(k, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length)
            v = rearrange(v, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length)
            modal_q = rearrange(
                modal_q, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length
            )
            modal_k = rearrange(
                modal_k, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length
            )
            modal_v = rearrange(
                modal_v, "b (n d l) c -> (b d) (n l) c", l=L, d=dilated_length
            )
        L_x = q.size(1)
        q_cats = torch.cat([q, modal_q], dim=1)
        k_cats = torch.cat([k, modal_k], dim=1)
        v_cats = torch.cat([v, modal_v], dim=1)
        attn_out = self.attn(
            q=q_cats,
            k=k_cats,
            v=v_cats,
        ...
        ## split
        x = attn_out[:, :L_x]
        modal_x = attn_out[:, L_x:]

        out = self.o(x)
        modal_out = modal_dit_layer.self_attn.o(modal_x)
        if use_dilated:
            out = rearrange(out, "(b d) (n l) c -> b (n d l) c", l=L, d=dilated_length)
            modal_out = rearrange(modal_out, "(b d) (n l) c -> b (n d l) c", l=L, d=dilated_length)
            if pad_len != 0:
                out = out[:, :-pad_len]
                modal_out = modal_out[:, :-pad_len]
        return out, modal_out
...
def _build_plan(full_dim: int, tp_rank: int, tp_size: int, tp_chunk_list=None):
    ...
    if len(tp_chunk_list) != tp_size:
        raise RuntimeError(
            f"len(tp_chunk_list)={len(tp_chunk_list)} != tp_size={tp_size}"
        )
...
class SelfAttentionTP(nn.Module):
    def __init__(
        ...
        self.q = ColumnParallelLinear(
            dim, dim, gather_output=False, tp_chunk_list=self.tp_chunk_list
        )
...
def _distribute_input_sp(x, context_group):
    ...
    assert N % context_group_world_size == 0, "cannot split tensor {} vs {}".format(
        x.shape, context_group_world_size
    )
    ...
    end_idx = (
        start_idx + local_dim
        if context_group_rank != context_group_world_size - 1
        else N
    )
...
class DiTBlock(nn.Module):
    def __init__(
        ...
        if world > 1 and (
            self.use_seq_parallel or use_tp_in_getaeddeltanet or use_tp_in_self_attn
        ):
        ...
        # if is_first_block:
        #     print(
        #         f"{self.__class__.__name__} use_seq_parallel: {use_seq_parallel} context_group_size: {self.context_group_size}"
        #     )
            ...
            if self.use_tp_in_self_attn and world > 1:
                ...
                if attend_k0:
                    raise NotImplementedError(
                        "SelfAttentionTP not support attend_k0 yet!!!"
                    )
                self.self_attn = SelfAttentionTP(
                    dim,
                    num_heads,
                    eps,
        ...
        self.ffn = nn.Sequential(
            nn.Linear(dim, ffn_dim), ACT2CLS["silu"](), nn.Linear(ffn_dim, dim)
        )
    ...
    def forward(self, x, context, t_mod, freqs, grid_size, context_mask=None):
        ...
        attn_out = self.cross_attn(
            self.cross_attn_norm(distributed_x), context, attn_mask=context_mask
        )
...
class KairosDiT(torch.nn.Module):
    def __init__(
        ...
        # print(f"{self.__class__.__name__} use_seq_parallel: {use_seq_parallel}, use_tp_in_getaeddeltanet: {use_tp_in_getaeddeltanet}, use_tp_in_self_attn: {use_tp_in_self_attn}")
